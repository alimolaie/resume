...
<div class="container-fluid menubala mb-3">
    <div class="row">
        ...
        <div class="col-6 col-sm-5 col-md-3 d-inline mt-3">
            <a href="#" class="btn-log animatebutton float-left">
                <p class="txt-log mr-1 d-inline vorod ">{{Auth::guard('member')->user()->user_name}}</p>
                <!-- fa fa-user-circle-o -->
                <i class="fa fa-user-o ml-1 d-inline-block icovord"></i>
            </a>
        </div>
    </div>
</div>
...
<div class="container" >
    <h3 style="color: #a80303;margin-top: 90px;" >
        مشاهده تیکت
    </h3>
    <hr/>
    ...
    <div class="row1 alert alert-info" >
        <div class="col-6">
            ...
            <p >
               <span class="badge badge-info"> بسته شده</span>
            </p>
            <p> بخش :   {{$ticket2->section}} </p>
        </div>
        <div class="col-6">
            ...
            <p>
                آخرین به روز رسانی : <span class="">{{$ticket2->updated_at}}</span>
            </p>
            <p>
               اولویت :
                {{$ticket2->priority}}
            </p>
            <a href="#" class="btn btn-danger">بستن تیکت</a>
        </div>

    </div>
    <div class="alert alert-info">
        <i class="fa fa-pencil" style="font-size:19px;color:rgb(68, 0, 255)"></i>
        پاسخ
    </div>
    @foreach($adminTicket as $admin)
    <div class="card">
        <div class="card-header" style="background: linear-gradient(135deg, #6297DB 50%, #1EECFF 100%);">
            <div class="float-right">
                <i class="fa fa-user-circle" style="font-size:26px;color:rgb(28, 3, 97)"></i>
                {{$admin->admin->name}}
                <b class="d-block mr-4">کارمندان</b>
            </div>
            @php
            ...
            <div class="float-left">
                <p class="mt-1">
                    {{\Morilog\Jalali\Jalalian::forge($admin->updated_at)}}
                </p>
            </div>
        </div>
        <div class="card-body">
        ...
        @if($admin->tiket_files!==null or $admin->tiket_files!=='' or $admin->tiket_files!==' ')
        <hr/>
        <div class="col-12  my-3">
            <label class="input-group">ضمیمه ها : </label>
            @foreach(explode(',', $admin->tiket_files) as $key=> $file)
            <a href="{{asset('uploads/admin-ticket/'.$file)}}" download ><i class="fa fa-download "></i><span>&nbsp;بارگیری  </span></a>
            @endforeach
        </div>
        @endif
    </div>
    ...
    <div class="card mt-3">
        <div class="card-header" style="background: linear-gradient(-135deg, #6297DB 50%, #1EECFF 100%);">
            <div class="float-right">
                <i class="fa fa-user-circle-o" style="font-size:26px;color:rgb(24, 24, 24)"></i>
                {{$user->member->first_name}} {{$user->member->last_name}}
                <b class="d-block mr-4">کاربر</b>
            </div>
            <div class="float-left">
                <p class="mt-1">
                      {{\Morilog\Jalali\Jalalian::forge($user->updated_at)}}
                </p>
            </div>
        </div>
        <div class="card-body">
        ...
        @if($user->tiket_files!==null or $user->tiket_files!=='' or $user->tiket_files!==' ')
        <hr/>
        <div class="col-12  my-3">
            <label class="input-group">ضمیمه ها : </label>
            @foreach(explode(',', $user->tiket_files) as $files)
                <a href="{{asset('uploads/tickets/'.$files)}}" download ><i class="fa fa-download "></i><span>&nbsp;بارگیری  </span></a>
            @endforeach
        </div>
@endif
    </div>
    @endforeach
    <div class="card my-3 ">
        <div class="card-header bg-light colorhed">
            <i class="fa fa-pencil" style="font-size:19px;color:rgb(68, 0, 255)"></i>
            پاسخ
        </div>
        <div class="card-body">
            <form method="post" action="{{route('viewTicket.send')}}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="member_id" value="{{Auth::guard('member')->id()}}">
                <input type="hidden" name="ticket_id" value="{{$ticket2->id}}">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-12 my-3">
                        ...
                        <textarea id="editor1" name="description" style="width: 100%;height: 200px"></textarea>
                    </div>
                    <div class="col-12 my-3">
                        <label class="input-group">ضمیمه ها : </label>
