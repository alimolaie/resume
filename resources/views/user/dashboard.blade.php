...
@php
    ...
    $countTicket=DB::table('tickets')->where('member_id',Auth::guard('member')->id())->count();
@endphp
<div class="container"  style="margin-top: 90px;">
    <div class="col-6">
        <div class="card-counter primary">
            <i class="fa fa-group"></i>
            <span class="count-numbers">{{$countTicket}}</span>
            <span class="count-name">تیکت</span>
        </div>
    </div>
    <br>
    <div class="list-group">
        <div class="list-group-item list-group-item-action active colorhed" >
            <i class="fa fa-comments" aria-hidden="true"></i>     تیکت های اخیر
            <a href="{{url('user/send-tiket')}}" class="btn btn-success  float-left" >درخواست جدید &nbsp;<i class="fa fa-plus" aria-hidden="true"></i></a>
        </div>
     @foreach($userTicket as $ticket)
            <div class="list-group-item list-group-item-action">
                <a href="{{url('user/show-tiket/'.$ticket->id)}}">{{$ticket->title}} <div class="badge badge-secondary">بسته شده</div></a>
                <span class="d-block">
           آخرین به روز رسانی در تاریخ :    {{\Morilog\Jalali\Jalalian::forge($ticket->updated_at)}}
               </span>
            </div>
        @endforeach
    </div>
